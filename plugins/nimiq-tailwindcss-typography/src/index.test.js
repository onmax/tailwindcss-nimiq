...
function run(config, plugin = tailwind) {
  ...
  return postcss(plugin(config)).process(
    ['@import url(../../../../fonts.css);', '@tailwind base;', '@tailwind components;', '@tailwind utilities'].join('\n'),
    {
    ...
    theme: {
      typography: {
        DEFAULT: {
          css: [
            {
              color: 'var(--nqtw-prose-body)',
              maxWidth: '65ch',
              '[class~="lead"]': {
                color: 'var(--nqtw-prose-lead)',
              },
              strong: {
                color: 'var(--nqtw-prose-bold)',
                fontWeight: '600',
              },
              'ol[type="A"]': {
